  ...
  return (
    <div style={{ background:'var(--navy-900)', border:'1px solid rgba(255,255,255,.05)', borderRadius:'var(--r)', overflow:'hidden' }}>
      ...
      <div style={{ padding:'8px 18px 12px' }}>
        {habits.map((h, i) => {
          const isRest = h.due_today === false && !h.logged_today
          return (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 0', borderBottom: i < habits.length-1 ? '1px solid rgba(255,255,255,.03)' : 'none', opacity: isRest ? 0.4 : 1 }}>
              <div style={{
              ...
              {isRest && (
                <span style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--muted)' }}>rest day</span>
              )}
              {h.streak > 0 && !isRest && (
